...
from ..utils.visualization import font_manager, get_display_text, create_title
    ...
    def visualize(self, output_file: str = None, layout: str = 'spring',
        ...
        for node in G.nodes():
            ...
            if entity:
                ...
                display_name = get_display_text(original_name)
                label = display_name[:10] + '...' if len(display_name) > 10 else display_name
                labels[node] = label
            else:
                labels[node] = node[:8]
        
        # 绘制标签
        font_props = font_manager.get_font_properties(size=8)
        nx.draw_networkx_labels(G, pos, labels, font_size=8, 
                               font_family=font_manager.available_font)
        
        # 创建标题
        title = create_title(len(G.nodes()), len(G.edges()))
        plt.title(title, fontsize=14, fontfamily=font_manager.available_font)
        plt.axis('off')
