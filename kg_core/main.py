...
from kg_core.entity_definition.entity_types import Entity, EntityTypes
from kg_core.entity_definition.relation_types import Relation, RelationTypes

from kg_core.knowledge_extraction.text_extractor import TextExtractor
...
from kg_core.knowledge_extraction.pattern_extractor import PatternExtractor
...
from kg_core.knowledge_mapping.ontology_mapper import OntologyMapper
from kg_core.knowledge_mapping.similarity_calculator import SimilarityCalculator
...
from kg_core.knowledge_fusion.conflict_resolution import ConflictResolver
...
class KnowledgeGraphDemo:
    ...
    def step1_ontology_definition(self):
        ...
        ontology_file = current_dir / "data" / "ontology.json"
        
        # 确保目录存在
        ontology_file.parent.mkdir(parents=True, exist_ok=True)
        
        self.ontology.export_ontology(str(ontology_file))
        print(f"📁 本体已导出到: {ontology_file}")
        
        print("✅ 主体定义完成!\n")
        return True
    
    def step2_knowledge_extraction(self, custom_texts=None):
        """步骤2: 知识抽取"""
        print("=" * 60)
        ...
        if custom_texts:
            ...
            print(f"📄 处理 {len(sample_texts)} 个自定义文本...")
        else:
