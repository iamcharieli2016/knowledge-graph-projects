...
class RelationExtractor:
    ...
    def _build_relation_patterns(self) -> Dict[str, List[Dict]]:
        ...
        patterns = {
            'works_for': [
                {'pattern': r'(\w+)(?:在|就职于|工作于|任职于)(\w+)', 'confidence': 0.9},
                {'pattern': r'(\w+)(?:是|担任)(\w+)(?:的|)(?:员工|职员)', 'confidence': 0.8},
                {'pattern': r'(\w+)(?:公司|企业)(?:的|)(\w+)', 'confidence': 0.7}
            ],
            'located_in': [
            ...
            'founder_of': [
                {'pattern': r'(\w+)(?:创建|创立|创办|成立)(?:了|)(\w+)', 'confidence': 0.9},
                {'pattern': r'(\w+)(?:是|)(\w+)(?:的|)(?:创始人|创办者)', 'confidence': 0.8}
            ],
            'parent_of': [
            ...
            for pattern_info in patterns:
                ...
                for match in matches:
                    if len(match.groups()) >= 2:
                        head_entity = match.group(1).strip()
                        tail_entity = match.group(2).strip()
                        
                        # 验证实体有效性
                        if self._is_valid_entity_pair(head_entity, tail_entity, entities):
